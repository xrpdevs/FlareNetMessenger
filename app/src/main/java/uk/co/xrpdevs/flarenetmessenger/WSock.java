...
public class WSock {
    ...
    private final static String TAG = WSock.class.getSimpleName();
    ...
    public static class Builder {
        ...
        public WSock build() {
            return new WSock(request.build());
        }
    }
     ...
     */
    public WSock onEvent(@NonNull String event, @NonNull OnEventListener listener) {
     ...
     * @param listener message on arrive listener
     */
    public WSock onEventResponse(@NonNull String event, @NonNull OnEventResponseListener listener) {
    ...
    /**
     * Set state listener which fired every time {@link WSock#state} changed.
     *
     * @param listener state change listener
     */
    public WSock setOnChangeStateListener(@NonNull OnStateChangeListener listener) {
     ...
     */
    public WSock setMessageListener(@NonNull OnMessageListener listener) {
     ...
     * {@link OnStateChangeListener#onChange(WSock, State)}
     *
    ...
    private void changeState(State newState) {
        ...
        if (onChangeStateListener != null) {
            onChangeStateListener.onChange(WSock.this, state);
        }
    }
    ...
    private void reconnect() {
        ...
        if (eventListener.get(EVENT_RECONNECT_ATTEMPT) != null) {
            eventListener.get(EVENT_RECONNECT_ATTEMPT).onMessage(WSock.this, EVENT_RECONNECT_ATTEMPT);
        }
    ...
    private final WebSocketListener webSocketListener = new WebSocketListener() {
        ...
        public void onOpen(WebSocket webSocket, Response response) {
            ...
            if (eventListener.get(EVENT_OPEN) != null) {
                eventListener.get(EVENT_OPEN).onMessage(WSock.this, EVENT_OPEN);
            }
        ...
        public void onMessage(WebSocket webSocket, String text) {
            ...
            if (messageListener != null)
                messageListener.onMessage(WSock.this, text);
...
//                if (eventResponseListener.get(event) != null) {
                ...
                Log.e(TAG, "Unknown message format. Error:\n" + e.getStackTrace());
        ...
        public void onClosed(WebSocket webSocket, int code, String reason) {
            ...
            if (eventListener.get(EVENT_CLOSED) != null) {
                eventListener.get(EVENT_CLOSED).onMessage(WSock.this, EVENT_CLOSED);
            }
        }
    ...
    public abstract static class OnMessageListener {
         ...
         */
        private void onMessage(WSock socket, final String data) {
    ...
    public abstract static class OnEventListener {
        ...
        private void onMessage(WSock socket, final String event) {
            new Handler(Looper.getMainLooper()).post(new Runnable() {
                @Override
                public void run() {
                    onMessage(event);
    ...
    public abstract static class OnEventResponseListener extends OnEventListener {
         ...
         */
        private void onMessage(WSock socket, final String event, final JSONObject data) {
            new Handler(Looper.getMainLooper()).post(new Runnable() {
                @Override
                public void run() {
    ...
    public abstract static class OnStateChangeListener {
         ...
         */
        private void onChange(WSock socket, final State status) {
            new Handler(Looper.getMainLooper()).post(new Runnable() {
                @Override
                public void run() {
                    onChange(status);
